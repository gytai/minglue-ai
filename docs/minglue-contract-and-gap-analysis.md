...
- **文档版本**：v1.5
- **文档更新时间**：2026-09-23
- **对应 issue**：GYTAI-127（接口规范盘点）/ 各阶段回写；v1.5 由 GYTAI-126（Stage 6 全链路验收）收口
...
> v1.5（Stage 6）说明：对**真实运行栈**（MySQL 8.0 / PostgreSQL 15 + Redis + uvicorn +
> 真实 HTTP）执行全链路验收，共 65 项端到端检查在双库上全部通过；同时修复 5 个只有
> 起真实服务才会暴露的缺陷（应用无法启动、干净安装登录 500、升级库结构不一致、
> 重复回调产生虚假失败审计、事件类型筛选串味）。全新验收结论、覆盖矩阵、缺陷清单
> 与剩余风险见 **§10**；可复现命令见根目录 `readme.md` 的「验收与回归」一节。
...
| F7 | 数据库迁移 / 唯一约束 / 新旧库升级测试 | 有：迁移脚本存在性与幂等性、`dedup_key` 回填、唯一约束在真实数据库层的效果（内存 SQLite）。**Stage 6 补**：初始化脚本与迁移脚本的列名覆盖一致性、`event_id` 扩宽、旧索引清理三条静态守卫；以及真实 MySQL/PG 上"Stage 1 结构 → 迁移 → 与新建库收敛"的实测比对（§10.3） | 已满足（Stage 6 收口） | — |
...
- ~~F7 迁移测试~~ → Stage 2 已补基础用例；Stage 6 补静态守卫与真实双库收敛比对（§5.7 F7、§10.3）
...
## 10. 全链路验收结论（Stage 6 / GYTAI-126，v1.5）

本节是 Stage 1–5 交付物的**独立验收**结果。与前几个阶段不同，本轮不满足于"测试通过"，
而是把系统真正跑起来：真实数据库（MySQL 8.0.46 / PostgreSQL 15.17）、真实 Redis、
真实 uvicorn 进程、真实 HTTP 调用，并对**全新初始化库**与**从 Stage 1 结构迁移上来的库**
分别验收。厂商侧由 `tests/e2e/run_acceptance.py` 内置的 mock HTTP 服务顶替，
**全程未向任何真实设备或生产厂商环境发送请求**。

### 10.1 执行环境与可复现命令

| 项 | 值 |
| --- | --- |
| 后端运行时 | Python 3.12.13 + `requirements.txt`（MySQL）/ `requirements-pg.txt`（PG）全量依赖 |
| 数据库 | MySQL 8.0.46（容器）、PostgreSQL 15.17（容器） |
| 缓存 | Redis 7（独立实例） |
| 厂商侧 | 脚本内置 mock，实现 §1 的 `accountLogin` 与 §4 的 5 个设备接口 |
| 验收脚本 | `dash-fastapi-backend/tests/e2e/run_acceptance.py`（只用标准库，65 项检查） |
| 复现步骤 | 见根目录 `readme.md` 的「验收与回归」一节 |

### 10.2 契约覆盖矩阵（最终）

对 §5 矩阵逐项复核后的结论。**状态定义**：已满足＝实现与契约一致且本轮有实证；
文档不明确＝厂商文档自身未定义，已固化为可配置/可观察行为，待真实联调确认。

| 分组 | 项数 | 状态 | 验收证据 |
| --- | --- | --- | --- |
| A 登录与鉴权（A1–A7） | 7 | 5 已满足 + A5/A7 文档不明确 | 真实登录成功/错误口令 601/无 token 401；mock 观测到密文密码、`isLock=true`、token 缓存命中（多次业务调用仅 1 次 `accountLogin`）、业务请求统一带 `Authorization: Bearer` |
| B 回调接口（B1–B19） | 19 | 18 已满足 + B17 文档不明确 | 8 类回调真实 HTTP 端到端入库、`duplicate_flag='Y'` 幂等、非法 JSON 422、未知类型不崩、签名默认关闭、`session_id`/`topic_name`/`duration`/`event_id` 落库、详情返回完整 `payload_json` |
| C 厂商设备控制（C1–C11） | 11 | 9 已满足 + C2 文档不明确 + C10 见下 | 5 个接口的参数位置与包络逐项断言；逐台 `results`/`partial`；缺项 SN 记为 `failed`；厂商业务错误码映射为可读 message；`msg_id` 留存与回查；401/403 只重试一次 |
| D 数据模型与 SQL（D1–D10） | 10 | 已满足 | 双库表结构、唯一键、索引一致；心跳字段落入台账；未知设备回调建档、配置同步不建档；超 5 分钟离线兜底可执行 |
| E 前端（E1–E12） | 12 | 已满足 | 前端进程启动、258 个回调注册成功、`_dash-layout`/`_dash-dependencies` 200；前端 API 层对运行中后端完成登录/设备/回调/控制日志真实调用；77 条前端测试；前后端权限与双库种子静态一致 |
| F 测试（F1–F9） | 9 | 已满足 | 后端 131 项 + 前端 77 项 + 端到端 65 项；F7 见 §10.3 |

**未覆盖项与原因**（全部为"需要真实环境才能定稿"，不是实现缺失）：

| 编号 | 未覆盖内容 | 原因 | 风险 |
| --- | --- | --- | --- |
| A5 / U1 | 鉴权头是否带 `Bearer ` | 厂商文档自相矛盾（§1.2） | 低；已做成 `MINGLUE_API_AUTH_SCHEME` 开关，联调时改配置即可 |
| A7 / U11 | 业务错误码取值表 | 文档未定义 | 低；当前透传 `message` |
| B17 / U10 | 其余 7 类回调的厂商重推策略 | 文档只定义了心跳响应 | 低；已把 `{"code":0}` 固化为 8 类一致并留完整原始报文便于重放 |
| C2 / U2 | 批量接口 query `sn` 是否真必填 | 文档标注与 sample 不一致 | 低；`MINGLUE_API_BATCH_SN_QUERY` 开关 |
| C10 备注 | 厂商**整批**返回业务错误码时无逐台结果 | 该场景厂商没有任何 `entities`，只能给批量级错误 | 中；已在 §4.6 R3 之外补充说明——批级错误与"缺项"是两种语义，前者整批失败、后者逐台记 `failed` |
| U3/U6–U9 | `config_data`、各类 `status`、指令日志 `payload`/`response` 结构 | 文档未定义 | 低；原样存储，不做字段级校验 |
| U12 | 厂商墙钟时区 | 文档未说明 | 中；`MINGLUE_VENDOR_TIMEZONE` 一处配置即可修正，联调时比对 |
| U13 | `_eof` 标记位置 | 文档模板与样例冲突 | 低；两种位置都识别 |
| U14 | 非 2xx 下的重推策略 | 文档未定义 | 低；401/503 均先留痕，可重放 |
| 真实设备控制 | 开始/停止录音的真实下发 | 遵守"不得对生产设备发起控制命令" | 中；必须先取得授权测试设备 |
...
| 步骤 | MySQL | PostgreSQL |
| --- | --- | --- |
| 路径 A：全新初始化 | `sql/dash-fastapi.sql`（23 张表） | `sql/dash-fastapi-pg.sql`（23 张表） |
| 路径 B：Stage 1 结构 + 迁移 | `27ed2e8` 版本初始化 → stage2 迁移 → stage5 迁移 | 同左 |
| 迁移幂等性 | 两个迁移脚本各连跑 2 次无报错、无重复种子 | 同左 |
| `ml_*` 列集合 | **98/98 完全一致**（列名、类型、可空性、默认值） | **98/98 完全一致** |
| `ml_*` 索引 | **20/20 完全一致** | **20/20 完全一致** |
| 设备菜单与角色授权 | 11 条菜单 + 11 条角色关联，两路径一致 | 同左 |
| 运行时 | 后端在路径 A 库启动并跑完 65 项验收 | 后端在**路径 B（迁移库）**启动并跑完 65 项验收 |

> 唯一差异是 `ml_*` 表的**列顺序**（迁移为追加列、初始化为内联定义）。列集合/类型/
> 可空性/默认值/索引均已核验一致，列顺序不影响 SQLAlchemy 按名映射与业务行为，属可接受差异。

> **MySQL 客户端字符集**：初始化脚本含中文种子数据，若客户端按 `latin1` 解析，
> `sys_notice.notice_title`（`varchar(50)`）会因字节数超长而中断导入，**导致其后所有
> `ml_*` 表静默缺失**。因此初始化/迁移命令必须带 `--default-character-set=utf8mb4`（已写入
> `readme.md`）。这是本轮发现的部署陷阱之一。

### 10.4 本轮发现并修复的缺陷

全部为"只在真实运行栈里才会暴露"的问题——前几个阶段的测试都在内存 SQLite + ASGI 传输层
里跑，绕过了生产会话配置与真实数据库方言。

| # | 严重度 | 缺陷 | 根因 | 影响 | 修复 |
| --- | --- | --- | --- | --- | --- |
| 1 | **P0** | 应用**完全无法启动** | `config/database.py` 在模块层声明了 `async_engine = None` / `AsyncSessionLocal = None` 哨兵属性；模块 `__getattr__` 只在常规查找失败时触发，哨兵使其永不生效，`from config.database import async_engine` 直接绑定 `None` | 启动即 `AttributeError: 'NoneType' object has no attribute 'begin'`，`init_create_table()` 失败，后端进程退出 | 删除哨兵，改用私有 `_lazy_engine_cache` 缓存惰性单例（`config/database.py`） |
| 2 | **P0** | 干净安装后**任何账号都无法登录** | `requirements.txt` 只锁 `passlib[bcrypt]==1.7.4` 而未锁 `bcrypt`；干净安装拉到 `bcrypt 5.0.0`，与 passlib 1.7.4 不兼容 | 登录返回 `code:500`，`msg` 为 `password cannot be longer than 72 bytes` | 两个 requirements 文件补 `bcrypt==4.0.1` |
| 3 | **P0** | 重复回调每次都多写一条**虚假失败审计** | 生产会话工厂未设 `expire_on_commit=False`，`commit()` 后读 `existing.callback_id` 触发同步惰性刷新 → `MissingGreenlet`；该错误又被 `_is_duplicate_conflict` 的裸子串匹配（命中异常回显 SQL 里的 `duplicate_flag` 列名）误判为"幂等冲突"并静默吞掉 | 每台设备每次重复心跳都留一条 `process_status='failed'` + 内部 ORM 错误，审计被污染且真实故障不可见；契约 B11 的承诺在真实栈上不成立 | ①生产会话工厂加 `expire_on_commit=False`；②重复分支在 commit 前取主键；③`_is_duplicate_conflict` 改为按 `IntegrityError` 类型判断，文本回退只认具体唯一约束措辞 |
| 4 | **P1** | 升级库与新建库**结构不一致**，长录音文件名在升级库上会落库失败 | stage2 迁移脚本漏了 `event_id` 从 `varchar(128)` 扩宽到 `varchar(255)`，也漏了清理被联合索引取代的旧索引 `idx_ml_callback_device`；初始化脚本改了、迁移脚本没同步 | 升级库插入长 `object_key`（文档样例已接近 128 字符，`ml_recording_file.object_key` 为 500）报 `Data too long for column 'event_id'`，整条回调落库失败并被厂商重推；两路径结构漂移 | 两个迁移脚本补 `event_id` 扩宽、补删旧索引、把 `dedup_key` 默认值对齐初始化脚本 |
| 5 | **P2** | 回调日志按类型筛选**串味** | `CallbackDao.get_list` 对 `event_type` 用了 `LIKE '%rec%'`，而 `process_status`/`duplicate_flag`/`session_id` 都是精确匹配 | 查 `rec` 会连 `reclist` 一起返回，运营排障时误读 | `event_type` 改为精确匹配（契约 §2 的封闭枚举，前端用固定选项筛选） |

修复 3 还额外收紧了幂等键冲突的判定口径：只有真正的唯一约束冲突才被当作"并发重推"吞掉，
其它异常一律向上抛出，避免"内部错误被伪装成正常重复回调"。

### 10.5 安全与脱敏核验

| 检查项 | 结果 |
| --- | --- |
| Apifox 访问密码 / 分享口令进入仓库 | ✅ 未发现（全仓扫描） |
| 厂商示例 AES 密钥进入仓库 | ✅ 未发现；文档仅保留掩码说明与密文用于溯源，测试使用合成密钥 `test-only-key-01` 并有静态扫描守卫 |
| 厂商凭证进入仓库 | ✅ `.env.dev`/`.env.prod` 的 `MINGLUE_API_USERNAME`/`PASSWORD`/`AES_KEY` 均为空，生产从密钥管理系统注入 |
| 生产环境回调签名默认状态 | ✅ `MINGLUE_CALLBACK_SECRET` 为空（厂商不签名，开启会让全部真实回调 401） |
| 回调日志接口泄露 token / 密钥 | ✅ 端到端断言通过（报文脱敏 + 值级抹除 + JWT/长 Base64 形态抹除 + 截断） |
| 错误信息回显请求报文 | ✅ 不回显（C11），mock 断言通过 |
| 敏感信息进入日志 | ✅ 控制日志不记录 token；登录请求只记录用户名与业务字段 |

> 残留：`.env.dev` / `.env.prod` 中的数据库口令为上游默认值（`mysqlroot` / `root`），
> 部署时必须替换。这属于模板占位，不构成凭证泄露，但需在部署流程中强制替换。

### 10.6 结论与剩余风险

- 契约 §5 矩阵中标记为「缺失」「部分满足」的项**已全部关闭**，P0/P1/P2 列表清空；
  剩余全部是「文档不明确 + 需真实环境确认」，集中在 §10.2 的未覆盖项表与 §6。
- 系统在 MySQL 与 PostgreSQL 上均可从零初始化、可从 Stage 1 结构平滑升级，
  后端与前端进程均能正常启动并完成端到端业务闭环。
- **最大剩余风险是"从未与真实设备/厂商环境联调"**：本文档 §6 的 14 条事项、
  §5.9 的 U1–U14，都必须用授权的测试设备与测试环境逐条确认。本轮已把它们整理成
  `readme.md` 的「联调清单」，可直接作为联调执行单。
...
| v1.5 | 2026-09-23 | CodeBuddy（Stage 6） | 全链路验收：真实 MySQL/PG + Redis + uvicorn 下 65 项端到端检查双库全通过；新增 §10（覆盖矩阵、双库初始化/升级收敛、缺陷清单、安全核验、剩余风险）；修复 5 个运行期缺陷（应用无法启动、干净安装登录 500、重复回调虚假失败审计、升级库 `event_id` 未扩宽与旧索引残留、`event_type` 筛选串味）；F7 收口；README 补齐部署/升级/环境变量/回调接入/联调清单/已知限制 |
