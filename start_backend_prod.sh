...
#   - 已配置 dash-fastapi-backend/.env.prod
#   - 数据库 (MySQL/PostgreSQL) 与 Redis 已启动
#
# 自动完成的步骤:
#   1. 在仓库根目录创建 .venv (若不存在)
#   2. 按需安装/更新前后端依赖
#   3. 切换到 dash-fastapi-backend/ 后以 prod 模式启动 uvicorn
#
# 数据库分支 (默认 MySQL):
#   REQUIREMENTS_FILE=requirements-pg.txt ./start_backend_prod.sh
#
# 可通过环境变量覆盖默认值:
...
#
# 示例:
#   APP_WORKERS=4 APP_ROOT_PATH=/prod-api ./start_backend_prod.sh
#
...
#
# 强制重装依赖:
#   rm -rf .venv/.deps_installed && ./start_backend_prod.sh
