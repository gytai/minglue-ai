...
#   - 已配置 dash-fastapi-frontend/.env.dev
#   - 后端已启动并监听在 .env.dev APP_BASE_URL 指向的地址
#
# 自动完成的步骤:
#   1. 在仓库根目录创建 .venv (若不存在)
#   2. 按需安装/更新前后端依赖 (与后端共享同一个 venv)
#   3. 切换到 dash-fastapi-frontend/ 后以 dev 模式启动 (Flask dev server + Dash 热重载)
#
# 覆盖端口/监听:
#   APP_HOST=127.0.0.1 APP_PORT=8088 ./start_frontend_dev.sh
#
# 强制重装依赖:
#   rm -rf .venv/.deps_installed && ./start_frontend_dev.sh
...
echo ">>> Minglue frontend starting (development)"
echo "    env    : ${APP_ENV}"
echo "    host   : ${HOST}"
echo "    port   : ${PORT}"
echo "    reload : enabled (Flask dev server + Dash)"
echo "    venv   : ${VENV_DIR}"
